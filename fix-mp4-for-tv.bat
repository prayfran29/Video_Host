...
for /r %%d in (*.mp4) do (
    if exist "%%d" (
        ...
        ffmpeg -i "%%d" -c:v libx264 -profile:v baseline -level 3.1 -c:a aac -ac 2 -b:a 128k -movflags +faststart+frag_keyframe+empty_moov -preset medium -crf 23 -r 30 -g 60 -keyint_min 60 -sc_threshold 0 -map 0:v:0 -map 0:a:0 -sn "!temp!"
